...
#Temporary Solution (Create An Empty IVR)
empty_dict = {
        "app_type": "ivr",
        "play_menu": "yes",
        "name": ivr_name,
...
create_ivr_payload = json.dumps(empty_dict)
...
def create_new_ivr():
    ...
    if response_create_ivr["status"]=="success":
        ...
        return False    
if(create_new_ivr()):
    pass #print("IVR Created successfully")

new_ivr_appid = response_create_ivr["response"]["app_id"]


###################ONWARDS UPDATE EMPTY DICT##############################
...
def action(input_action):
    ...
    if input_action == 1:    
        ...
        "action": "call",
        "to": "user",
        "id": input("enter user_id which you want to connect with IVR digit")
        }
        return ph_var
    if input_action == 2:
        ph_var= {
...
def update_ivr():
    ...
    update_ivr_response = requests.request("PUT", "{}account/{}/voiceapp/{}".format(base_url, acc_id, new_ivr_appid), headers=update_ivr_headers, data=update_ivr_payload).json()
